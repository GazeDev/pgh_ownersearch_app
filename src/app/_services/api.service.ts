...
export class ApiService {
  ...
  */
  ...
  // public webSockets: WebSocketSubject = webSocket('ws://localhost:23089/multiple');
  getMultiple() {
    return this._webSocket('/multiple').asObservable();
  }

  sendMultiple(obj) {
    return this._webSocket('/multiple').next(obj);
  }

  getEvents() {
    this._webSocket('/getSearchEvents').next({});
    return this._webSocket('/getSearchEvents').asObservable();
  }
